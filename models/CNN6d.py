...
class CNN6d(nn.Module):
    def __init__(self):
        ...
        self.body = nn.ModuleList([
            ...
            nn.Sequential(OrderedDict([
                ('layer', nn.Conv2d(12, 20, kernel_size=3, padding=1, stride=2, bias=False)),
                ('act', act)
            ])),
            nn.Sequential(OrderedDict([
                ('layer', nn.Conv2d(20, 36, kernel_size=3, padding=1, stride=1, bias=False)),
                ('act', act)
            ])),
            nn.Sequential(OrderedDict([
